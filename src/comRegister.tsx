...
class ComRegister {
    ...
    private async checkDynamic(ctx: Context) {
        try{
            ...
            let prevTime = await ctx.database.select('bili_check', {}).execute(row=>$.max(row.startTime))
            if(!prevTime) prevTime = new Date()
            ...
            await ctx.database.set('bili_check', {
                ...
                num_total, num_followed, num_sent, endTime, num_filtered: num_new
            })
        } catch(e){
            console.log(e)
        }
    ...
    private async initBiliCmd(ctx: Context) {
        ...
        biliCom
            ...
            .action(async ({ session, options }, mid, ...targets) => {
                ...
                // 将该用户移动到关注分组
                ctx.ba.checkFollow(mid).then(async (data) => {
                    const followed = [2,6].includes(data.data.attribute)
                    if (!followed) {
                        await ctx.ba.followUser(mid)
                    }
                    await ctx.ba.cpToGroup([mid], [this.followGroup])
                })
                // 获取直播房间号
    ...
    private async setupFollowGroup(ctx: Context) {
        try {
            ...
            if(egroup){
                ...
                this.logger.error('未找到关注分组：' + gname)
                const resp = await ctx.ba.createFollowGroup(gname)
                if (resp.code === 0) {
                    this.followGroup = resp.data.tagid
                } else {
                    this.logger.error('创建关注分组失败，错误为：' + resp.message)
                }
            }
        } catch (e) {
            this.logger.error(e.message)
        } finally {
            this.logger.info('设置关注分组为：' + this.followGroup)
        }
    }
    ...
    private async checkUnsubUser(ctx: Context, uid: number) {
        ...
        if(subNum===0){
            ...
            if(this.config.followGroup.length > 0)
