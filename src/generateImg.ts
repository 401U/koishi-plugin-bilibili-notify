...
class GenerateImg extends Service {
    ...
    protected moduleHandler: Record<string, (data: any, forward: boolean)=>RenderSection>

    protected register(name: string | string[], handler: (data: any, forward: boolean)=>RenderSection) {
        if (typeof name === 'string') {
            name = [name]
        }
        name.forEach(n => this.moduleHandler[n] = handler)
    }
    ...
    protected doRegister() {
        this.register(ADDITIONAL_TYPE_RESERVE, data=>{
        ...
        const renderBasicDynamic = (data: DynamicItemDynamicWrapper) => {

            let main: string = ''
            // const module_dynamic = dynamicMajorData.modules.module_dynamic
            if (data.desc) {
                const richText = data.desc.rich_text_nodes.reduce((accumulator, currentValue) => {
            ...
            if (data.major && data.major.draw) {
                if (data.major.draw.items.length === 1) {
                    major += `<img class="single-photo-item" src="${data.major.draw.items[0].src}"/>`
                } else if (data.major.draw.items.length === 4) {
                    major += data.major.draw.items.reduce((acc, cV) => {
        ...
        this.register([DYNAMIC_TYPE_WORD, DYNAMIC_TYPE_DRAW, DYNAMIC_TYPE_FORWARD], (data) => {
            const dynamicMajorData: DynamicItem = data
            let main: string = renderBasicDynamic(dynamicMajorData.modules.module_dynamic)
            ...
            if (dynamicMajorData.type === DYNAMIC_TYPE_FORWARD) {
                ...
                // 获取转发的动态
                const [forwardMain, _, forwardInfo] = this.renderMajor(dynamicMajorData.orig, true)
                // 拼接main
                main += `
            ...
            if (additional) {
                // 有附加信息，判断类型
                let additonalHandler = this.moduleHandler[additional.type]
                if (additonalHandler) {
                    main+=additonalHandler(additional, true)
                }
            }
            return {content: main}
        })
        this.register(DYNAMIC_TYPE_AV, (data, forward) => {
            let dynamicMajorData: DynamicItem = data
            // 处理文字
            let main = renderBasicDynamic(data)
            let forwardInfo: string
            let pubTime = this.unixTimestampToString(dynamicMajorData.modules.module_author.pub_ts)
            ...
            `
            let link = `请将$替换为. www$bilibili$com/video/${archive.bvid}`
            return { content:main, link }
            
        })
    }

    protected renderMajor(data: DynamicItem, forward: boolean) {
        // 定义返回值
        let main: RenderSection
        let moduleHandler = this.moduleHandler[data.type]
        if (moduleHandler) {
            main = moduleHandler(data, forward)
        }
        return main
    }

    render(data: any, forward: boolean) {
        return ''
    }
    ...
    async generateLiveImg(data: any, userData: any, liveStatus: number /*0未开播 1刚开播 2已开播 */) {
        ...
        for (let i = 0; i < attempts; i++) {
            try {
                // 判断渲染方式
                if (this.giConfig.renderType==='page') {
    ...
    async generateDynamicImg(data: DynamicItem) {
        ...
        const getDynamicMajor = (dynamicMajorData: DynamicItem, forward: boolean): [string, string, string?] => {
            // 定义返回值
            let main: string = ''
            let link: string = ''
            // 定义forward类型返回值
            ...
            switch (dynamicMajorData.type) {
                ...
                case DYNAMIC_TYPE_FORWARD: {
                    ...
                    if (dynamicMajorData.type === DYNAMIC_TYPE_FORWARD) {
                        ...
                        const forwardUserName = forward_module_author.name
                        // 获取转发的动态
                        const [forwardMain, _, forwardInfo] = getDynamicMajor(dynamicMajorData.orig, true)
                        // 拼接main
                        main += `
                        <div class="card-forward">
                            <div class="forward-userinfo">
                    ...
                    if (dynamicMajorData.modules.module_dynamic.additional) {
                        ...
                        let additonalHandler = this.moduleHandler[additional.type]
                        if (additonalHandler) {
                            main+=additonalHandler(additional, forward)
                        }
                    }
        ...
        // 获取动态主要内容
        const [main, link] = getDynamicMajor(data, false)
        // 加载字体
        const fontURL = pathToFileURL(resolve(__dirname, 'font/HYZhengYuan-75W.ttf'))
        // 判断是否开启大字体模式
        let style: string
    ...
    private async getTimeDifference(dateString: string) {
        // 将日期字符串转换为Date对象
    ...
    private unixTimestampToString(timestamp: number) {
