...
type RENDER_TYPES = DYNAMIC_TYPES | ADDITIONAL_TYPES
...
class Render extends Service{
     ...
     * 针对不同的渲染类型进行渲染的函数map
     */
    private renderHandler: Map<RENDER_TYPES, RenderFunc> = new Map()

    /**
     * 注册渲染函数
     * @param name 一个字符串或者一个字符串数组， 表示渲染函数处理的模块类型 
     * @param handler 对应的渲染函数
     */
    private registerRenderHandler(name: RENDER_TYPES | RENDER_TYPES[], handler: RenderFunc) {
        if(typeof name === 'string') {
            name = [name]
        }
        name.forEach(n => {
            this.renderHandler.set(n, handler)
        })
    }
    ...
    private setupHandlers() {
        this.registerRenderHandler(['DYNAMIC_TYPE_WORD', 'DYNAMIC_TYPE_DRAW', 'DYNAMIC_TYPE_FORWARD'], (data, forward) => {
            ...
            if(data.type === 'DYNAMIC_TYPE_FORWARD') {
                ...
                try {
                    let forwardResult = this.renderMajor(item.orig, true)
                    forwardContent = forwardResult.content
                    forwardText = forwardResult.forwardText
                }catch(e) {
                    forwardContent = e.message
                }
                content += `
                    <div class="card-forward">
                        <div class="forward-userinfo">
                            <img class="forward-avatar" src="${forwardAuthor.face}" alt="avatar">
                            <span class="forward-username">${forwardAuthor.name} ${forwardText}</span>
                        </div>
            ...
            if(additional){
                let additionalHandler = this.renderHandler.get(additional.type as RENDER_TYPES)
                if(additionalHandler) {
                    try{
                        content += additionalHandler(additional, forward)
                    }catch(e) {
                        this.log.error('附加内容渲染出错', e)
                    }
                }
            }
            return {content}
        })
        this.registerRenderHandler('DYNAMIC_TYPE_AV', (data, forward) => {
            let item = data as DynamicItem
            let content = this.renderDynamicModule(item.modules.module_dynamic)
            let forwardText = ''
        ...
        this.registerRenderHandler([
            ...
            return {content: `${item.modules.module_author.name}发布了新的${typeMap[item.type]}, 请自行查看`}
        })
        this.registerRenderHandler('ADDITIONAL_TYPE_RESERVE', (data)=>{
            const reserve = (data as AdditionalModuleWrapper).reserve
            let button: string
            // 判断按钮类型
    ...
    private renderDynamicModule(module: DynamicItemDynamicWrapper){
        let main: string = ''
        if (module.desc) {
            const richText = module.desc.rich_text_nodes.reduce((accumulator, currentValue) => {
        ...
        if (module.major && module.major.draw) {
            if (module.major.draw.items.length === 1) {
                major += `<img class="single-photo-item" src="${module.major.draw.items[0].src}"/>`
            } else if (module.major.draw.items.length === 4) {
                major += module.major.draw.items.reduce((acc, cV) => {
    ...
    private renderMajor(data: DynamicItem, forward: boolean): ModuleRenderResult {
        const handler = this.renderHandler.get(data.type as RENDER_TYPES)
        if(!handler) {
            throw new Error('未找到对应的渲染函数')
        }
    ...
    async render(data: DynamicItem): Promise<h.Fragment>{
        ...
        while(attempt > 0) {
            attempt--
            try {
                if(this.conf.renderType === 'page'){
                    const buffer = await this.renderByPage(html)
                    return h.image(buffer, 'image/png')
                } else {
                    const pic = await this.renderByRender(html)
                    return pic
                }
            } catch (error) {
                this.log.warn('生成图片失败, 剩余尝试次数：'+attempt, error)
            }
        }
    ...
    private async renderByRender(html: string){
        return await this.ctx.puppeteer.render(html)
    }

    /**
     * 加载样式文件
     */
    private loadStyleText(dynamicCardColor: string, dynamicCardUrl: string) {
        let style: string
        const fontURL = pathToFileURL(resolve(__dirname, 'font/HYZhengYuan-75W.ttf'))
        if (this.conf.largeFont) {
            style = `
            @font-face {
            ...
            * {
                ...
                font-family: "${this.conf.font}", "Custom Font", "Microsoft YaHei", "Source Han Sans", "Noto Sans CJK", sans-serif;
            }
            ...
            .background {
                ...
                height: auto;
                padding: 15px;
                background: linear-gradient(to right bottom, ${this.conf.cardColorStart}, ${this.conf.cardColorEnd});
                overflow: hidden;
            }
    ...
    private bundleHtml(data: DynamicItem, renderResult: ModuleRenderResult, style: string) {
        const { module_author: authorModule, module_dynamic: dynamicModule, module_stat: statModule } = data.modules
        let pubTime = this.unixTsToStr(authorModule.pub_ts)

        let cardUrl: string, cardColor: string, cardId: number
        if (authorModule.decorate) {
            cardUrl = authorModule.decorate.card_url
            cardColor = authorModule.decorate.fan.color
            cardId = authorModule.decorate.fan.num_str
        }
        const topic = dynamicModule.topic ? dynamicModule.topic.name : ''
        return `
            <!DOCTYPE html>
            <html>
            <head>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <img class="anchor-avatar"
                                    src="${authorModule.face}"
                                    alt="主播头像">
                                <div class="card-content">
                                    <div class="card-header">
                                        <div class="up-info">
                                            <div class="up-name" style="${authorModule.vip.type !== 0 ? 'color: #FB7299' : ''}">${authorModule.name}</div>
                                            <div class="pub-time">${pubTime}</div>
                                        </div>
                                        ${authorModule.decorate ? `
                                        <div class="dress-up">
                                            <img src="${cardUrl}" />
                                            <span>${cardId}</span>
                                        </div>
                                    ...
                                    <div class="card-stat">
                                        <div class="stat-item">
                                            ...
                                            <span>${statModule.forward.count}</span>
                                        </div>
                                        <div class="stat-item">
                                            <svg style="width: 18px; height: 18px;" xmlns="http://www.w3.org/2000/svg"
                                                xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 18 18" width="18"
                                            ...
                                            <span>${statModule.comment.count}</span>
                                        </div>
                                        <div class="stat-item">
                                            <svg style="width: 18px; height: 18px;" xmlns="http://www.w3.org/2000/svg"
                                                xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 18 18" width="18"
                                            ...
                                            <span>${statModule.like.count}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
namespace Render {
    ...
    export const Config: Schema<Config> = Schema.object({
        ...
        noBorder: Schema.boolean().default(false).description('移除推送卡片边框'),
